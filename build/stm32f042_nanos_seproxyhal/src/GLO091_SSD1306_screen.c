...
//#define HAVE_GLO091_BLACK_ON_WHITE

// SSD1306 is paginated a byte send to the device is a vertical slice of 8 bits of bitmap. lsb is the lower y.
...
const unsigned char screen_init_commands[] = {
  ...
  0x40, // initial line
/*
  0xD5, // set refresh clock
  0xF0, // max freq, divide ratio 1
*/
...
unsigned int screen_animation_ongoing;
void screen_init(unsigned char reinit)
{      
  unsigned int i; 

  screen_animation_ongoing = 0;

}

unsigned int screen_inverted;
unsigned int screen_angle;
unsigned int screen_luminosity;
...
/*
  ...
  __asm volatile ("mrs %0, PRIMASK":"=r"(it_dis));
  __asm volatile ("cpsid i");
*/  
  if (screen_changed /*&& ! screen_animation_ongoing*/) {
    ...
    lcd_control(screen_luminosity*255UL/100UL); // level value
...
/*
  // reenable it only if enabled when entering
  if (!it_dis) {
...
void screen_animate(unsigned int animation_kind, unsigned char* parameters) {
  unsigned int _x,_y,ystep,_ymin,_ymax;
  unsigned int it_dis;
  switch (animation_kind) {
      case SEPROXYHAL_TAG_SCREEN_ANIMATION_STATUS_VERTICAL_SPLIT_SLIDE: {
...
*/     
        ...
        // decode parameters
        unsigned int ysplit = U2BE(parameters, 0);
        unsigned int duration = U2BE(parameters, 2);
  
#define YSPLIT ysplit //19L
#define ANIM_DURATION_MS duration // 150
        extern unsigned int uwTick;
        unsigned int start_ms = uwTick;
        unsigned int up_last_y = -1;
        unsigned int up_last_rshift=0;
        unsigned int up_last_mask=0;
        while(uwTick < start_ms + ANIM_DURATION_MS) {
#define DN_YSPLIT (YSPLIT)

            unsigned int dn_last_y=0;
            unsigned int dn_last_rshift=0;
            unsigned int dn_last_mask=0;
  


            // part with y<ysplit is slided with a +dy speed
#define DN_YSPLIT (YSPLIT)
            ystep = (DN_YSPLIT)*(uwTick-start_ms)/ANIM_DURATION_MS;
            _ymax = (ystep/8); 
            // number of page of 8 lines to be modified for the current step
            for (_y=0; _y<=_ymax; _y++) { 
                lcd_control(0xB0|_y);              // <- page address
                unsigned int src_y = DN_YSPLIT/8+1/*(UP_YSPLIT%8?1:0)*/+(_y-_ymax-1);
                unsigned int idx = src_y*SCREEN_WIDTH;
                dn_last_mask = ((_y!=_ymax))?0xFF:(0xFF>>(8-(ystep)%8));
                dn_last_rshift = DN_YSPLIT%8+1+(8-ystep%8);
                for (_x=0; _x<SCREEN_WIDTH; _x++) {
                    unsigned int val = ((src_y<((SCREEN_HEIGHT/8)-1))?(screen_framebuffer[idx+_x+SCREEN_WIDTH]<<16):0) | (screen_framebuffer[idx+_x]<<8) | ((src_y>0)?(screen_framebuffer[idx+_x-SCREEN_WIDTH]):0) ;
                    unsigned int v = (val>>(dn_last_rshift))&dn_last_mask;
                    if (_y == up_last_y) {
                      v |= (val>>(up_last_rshift))&up_last_mask;
                    }
                    lcd_write(v);
                }
            }
            ...
            // part with y>=ysplit is slided with a -dy speed
#define UP_YSPLIT (YSPLIT)
            ystep = (SCREEN_HEIGHT-UP_YSPLIT)*(uwTick-start_ms)/ANIM_DURATION_MS;
            _ymin = SCREEN_HEIGHT/8-((ystep/8)+1); 
            // number of page of 8 lines to be modified for the current step
            for (_y=SCREEN_HEIGHT/8-1; _y>=_ymin; _y--) { 
                lcd_control(0xB0|_y);              // <- page address
                unsigned int src_y = UP_YSPLIT/8+1/*(UP_YSPLIT%8?1:0)*/+(_y-_ymin-1);
                unsigned int idx = src_y*SCREEN_WIDTH;
                up_last_mask = ((_y!=_ymin))?0xFF:(0xFF<<(8-(ystep)%8));
                up_last_rshift = UP_YSPLIT%8+1+ystep%8;
                for (_x=0; _x<SCREEN_WIDTH; _x++) {
                    unsigned int val = ((src_y<((SCREEN_HEIGHT/8)-1))?(screen_framebuffer[idx+_x+SCREEN_WIDTH]<<16):0) | (screen_framebuffer[idx+_x]<<8) | ((src_y>0)?(screen_framebuffer[idx+_x-SCREEN_WIDTH]):0) ;
                    unsigned int v = (val>>(up_last_rshift))&up_last_mask;
                    if (_y == dn_last_y) {
                      v |= (val>>(dn_last_rshift/*+8*/))&dn_last_mask;
                    }
                    lcd_write(v);
                }
            }
        ...
        // perform a last screen update to ensure no step missed (even if the animation speed is too fast)
        screen_update();
        break;
      }
  }
